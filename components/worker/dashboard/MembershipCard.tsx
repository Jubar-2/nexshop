...
import { AlertCircle, Medal, ShieldCheck, Users, Zap } from "lucide-react";
            ...
            <Card className="bg-white border-slate-100 shadow-sm rounded-2xl overflow-hidden flex flex-col">
                ...
                <CardContent className="pt-6 space-y-5">
                    ...
                    <div className="space-y-3">
                        <Skeleton className="h-5 w-34.5 bg-slate-200" />
                        <Skeleton className="h-5 w-34.5 bg-slate-200" />
                        <Skeleton className="h-5 w-34.5 bg-slate-200" />
                    </div>
                </CardContent>
    ...
    return (
        <Card className="bg-white border-slate-100 shadow-sm rounded-2xl overflow-hidden flex flex-col">
            ...
            <CardContent className="pt-6 space-y-5">
                <div className="p-4 bg-slate-50 rounded-2xl border border-slate-100">
                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Active Plan</p>
                    <p className="text-2xl font-black text-slate-800">{data?.membershipPlan?.membershipName}</p>
                </div>
                <div className="space-y-3">
                    {data?.membershipPlan?.membershipName?.offers?.map(() => {
