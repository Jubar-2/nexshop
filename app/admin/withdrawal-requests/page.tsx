...
// --- SUB-COMPONENTS ---
type AdminSummaryCardType = {
    label: string, val: string, color: string, icon: ReactNode
}
const AdminSummaryCard = ({ label, val, color, icon }: AdminSummaryCardType) => {
    ...
    return (
        <div className="bg-white px-5 py-3 rounded-2xl shadow-sm flex items-center gap-4 min-w-[160px] border border-slate-100">
            <div className={`p-2 rounded-lg ${colors[color as "emerald" | "amber"]}`}>{icon}</div>
            <div>
                <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest leading-none mb-1">{label}</p>
                <p className={`text-lg font-black tracking-tighter leading-none ${colors[color as "emerald" | "amber"].split(' ')[0]}`}>{val}</p>
            </div>
        </div>
    );
};
...
const StatusBadge = ({ status }: { status: string }) => {
    ...
    return (
        <Badge variant="outline" className={`${styles[status as "Pending" | "Approved" | "Rejected"]} border-none font-black text-[9px] uppercase tracking-widest px-2.5 py-1 rounded-lg mx-auto`}>
            {status}
        </Badge>
    );
};