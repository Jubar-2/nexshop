...
import { useGetJob } from '@/hooks/admin/use-jobs';
...
export default function AdminUpdateJob() {
    ...
    useEffect(() => {
        if (!data || !categories || categories.length === 0) return;
        // console.log(data)
        reset({
            ...
            reward: data?.reward,
            workerRequired: data?.workerRequired,
            category: "",
            subCategory: ""
        });
        ...
        const timer = setTimeout(() => {
            if (data?.category?.id) {
                ...
                if (data?.subCategory?.id) {
                    setValue("subCategory", data.subCategory.id, { shouldValidate: true });
    ...
    }, [data, categories, reset, setValue]);

    useEffect(() => {
        if (watchedCategory && currentCategoryData?.subCategories?.length > 0) {

            const incomingSubCatId = data?.subCategory?.id || data?.subCategory;

            if (!incomingSubCatId) return;

            const isIdValid = currentCategoryData.subCategories.some(
                (sub: Category) => sub.id === incomingSubCatId
            );

            if (isIdValid) {
                const timer = setTimeout(() => {
                    setValue("subCategory", incomingSubCatId, { shouldValidate: true });
                }, 60);

                return () => clearTimeout(timer);
            }
        }
    }, [watchedCategory, currentCategoryData, data, setValue]);

    if (isLoading) return <div className="p-20 text-center font-bold">Loading Job Data...</div>;
