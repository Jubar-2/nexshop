...
type InfoItemType = {
    icon: ReactNode, label: string, val: string
}

const InfoItem = ({ icon, label, val }: InfoItemType) => (
...
const StatMini = ({ label, val, icon }: StatMiniType) => (
...
const RecentWorkRow = ({ title, date, reward, status }: RecentWorkRowType) => (
    <div className="px-8 py-4 flex items-center justify-between hover:bg-slate-50/50 transition-colors cursor-pointer group">
        ...
        <div className="text-right">
            ...
            <span className={`text-[9px] font-black uppercase px-1.5 py-0.5 rounded ${status === 'Approved' ? 'bg-emerald-100 text-emerald-700' : 'bg-amber-100 text-amber-700'
                }`}>
                {status}
            </span>
        </div>
