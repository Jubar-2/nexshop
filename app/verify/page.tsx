...
import { useGetUnverified, useResendEmail, useVerifyEmail } from "@/hooks/use-freelancer";
...
export default function OTPVerificationPage() {
  ...
  const { data, isLoading } = useGetUnverified();
  ...
  useEffect(() => {
    if (!data?.verifyCodeExpiry) return;
    setCanResend(false);
    const diff =
      new Date(data.verifyCodeExpiry).getTime() - Date.now();
        ...
        <CardHeader className="pt-10 pb-6 text-center space-y-4">
          ...
          <div className="space-y-1">
            ...
            <CardDescription className="text-slate-500 font-medium">
              We sent a 6-digit code to <span className="text-slate-900 font-bold">{isLoading || data?.email}</span>
            </CardDescription>
          </div>
        </CardHeader>
