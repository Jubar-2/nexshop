...
export async function PATCH(request: Request): Promise<Response> {
    try {
        ...
        if(notificationCount === 0) {
            return ApiResponse.success({ message: "No unread notifications to mark as read." }, "No unread notifications found");
        }
        ...
        const seeMessage = await db.notification.updateMany({
            where: {
                userId: userId as string,
                read: false,
            },
        ...
        return ApiResponse.success(seeMessage, "Notification history retrieved successfully");
