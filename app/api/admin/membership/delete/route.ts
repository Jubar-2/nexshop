...
export async function DELETE(request: Request) {
    try {
        ...
        const deleted = await prisma.membershipPlan.deleteMany({
            where: {
                id: { in: ids }
            }
        });

        return ApiResponse.success(deleted, "Membership Plan delete successfully", 201);

    } catch (error: unknown) {

        console.error("JOB_CREATE_ERROR:", error);

        return ApiResponse.error("Failed to delete Membership Plan", 500);
    }
}