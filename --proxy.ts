...
export async function proxy(request: NextRequest) {
  ...
  const protections = [
    ...
    { path: "/signin", role: "FREELANCER" },
    { path: "/signup", role: "FREELANCER" },
  ];
  ...
  if (activeRoute) {
    ...
    if (!token) {
      ...
      if (activeRoute.path === "/signin" || activeRoute.path === "/signup") {
    ...
    if (activeRoute.path === "/signin" || activeRoute.path === "/signup") {
      // If user is already authenticated, redirect them away from signin/signup
